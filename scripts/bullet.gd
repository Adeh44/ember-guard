# ============================================================
# bullet.gd — le projectile
# Vole en ligne droite, s'arrête au premier obstacle, applique
# les dégâts, génère le bruit d'impact, puis disparaît.
# Ses stats sont injectées par player.gd au moment du tir.
# ============================================================
...
# ========== INJECTÉES PAR player.gd AU MOMENT DU TIR ==========
var direction = Vector2.RIGHT   # Sens du vol (dispersion du recul déjà appliquée)
var crit_chance = 0.05          # Chance de critique calculée pendant la visée

# ========== INTERNE ==========
var distance_travelled = 0.0    # Distance parcourue depuis le tir

func _ready():
	# Oriente le sprite dans le sens du tir (il pointe vers la droite au repos)
...
func _on_impact(collision):
	...
	print("IMPACT balle sur : ", body.name)   # DEBUG : à retirer quand tout sera validé

	# get_position() = le point de contact EXACT, en coordonnées globales.
	# Plus juste que global_position pour le système de propagation sonore.
	...
	# Mur ou ennemi : la balle est consommée dans les deux cas
	queue_free()
